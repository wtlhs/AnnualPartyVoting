...
import { createQRCodeURLGenerator, QRCodeHelpers } from '../utils/qr-code-url-generator';
 ...
 */
export async function demonstrateQRCodeGeneration() {
  ...
  try {
    ...
    const printQR = await QRCodeHelpers.generateForPrint(generator, candidate);
    ...
    const customOptions: QRCodeGenerationOptions = {
      format: 'png',
      size: 300,
      margin: 3,
      errorCorrection: 'H',
    ...
    const multipleFormats = await QRCodeHelpers.generateMultipleFormats(
      generator, 
      candidate, 
      ['wechat', 'web', 'small']
    );
    ...
    const validOptions: QRCodeGenerationOptions = {
      size: 256,
      margin: 2,
      quality: 0.9,
      darkColor: '#000000',
    ...
    const validationResult = QRCodeHelpers.validateOptions(validOptions);
    ...
    const invalidOptions: QRCodeGenerationOptions = {
      size: 32, // Too small
      margin: -1, // Negative
      quality: 1.5, // Too high
      darkColor: 'invalid-color'
    };
    const invalidValidation = QRCodeHelpers.validateOptions(invalidOptions);
 ...
 */
export function getWeChatOptimizationTips(): string[] {
  return [
    '使用高错误纠正级别 (H) 提高扫码成功率',
 ...
 */
export function getPrintOptimizationTips(): string[] {
  return [
...
export default demonstrateQRCodeGeneration;