 ...
 */
export class VotingAuthenticationGuard implements AuthenticationGuard {
   ...
   */
  async requireAuthentication(): Promise<User | null> {
   ...
   */
  async checkVotingPermissions(): Promise<boolean> {
   ...
   */
  async handleSessionExpiry(redirectToLogin: boolean = false): Promise<void> {
    await this.clearSession();
   ...
   */
  private async refreshUserInfo(userId: string): Promise<User | null> {
    try {
   ...
   */
  private async validateWithServer(user: User): Promise<{
    isValid: boolean;
    user?: User;
    token?: string;
    expiresAt?: Date;
  ...
  private persistSession(): void {
    try {
      ...
      localStorage.setItem(
        ...
        JSON.stringify(this.currentSession.user)
      );
 ...
 */
export function createAuthenticationGuard(config?: Partial<AuthGuardConfig>): AuthenticationGuard {
 ...
 */
export const authGuard = createAuthenticationGuard();
...
export const AuthUtils = {
   ...
   */
  async requireAuthWithRedirect(returnUrl?: string): Promise<User | null> {
    const user = await authGuard.requireAuthentication();
    if (!user) {
      authGuard.redirectToLogin(returnUrl || window.location.href);
      return null;
    }
    return user;
  },
   ...
   */
  async checkVotingPermissions(): Promise<{ canVote: boolean; reason?: string }> {
    const user = await authGuard.requireAuthentication();
    ...
    if (!user.hasVotingRights) {
      return { 
        canVote: false, 
        reason: ERROR_MESSAGES.VOTING_PERMISSION_DENIED 
      };
    }
